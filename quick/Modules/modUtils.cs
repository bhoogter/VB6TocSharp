...
static class modUtils
{
    ...
    {
        int _StrCnt = (Len(Src) - Len(Replace(Src, Str, ""))) / (int)IIf(Len(Str) == 0, 1, Len(Str));
        return _StrCnt;
    }
    ...
    public static dynamic ArrSlice(ref dynamic sourceArray, int fromIndex, int toIndex)
    {
        ...
        {
            ArrAdd(ref tempList, sourceArray(Idx));
        }
        _ArrSlice = tempList;
        return _ArrSlice;
    }
    ...
    public static string cVal(ref Collection Coll, string Key, string Def = "")
    {
        ...
        // TODO: (NOT SUPPORTED): On Error Resume Next
        _cVal = Def;
        _cVal = (string)Coll[LCase(Key)];
        return _cVal;
    }
    public static string cValP(Collection Coll, string Key, string Def = "")
    {
        return P(deQuote(cVal(ref Coll, Key, Def)));
    }
